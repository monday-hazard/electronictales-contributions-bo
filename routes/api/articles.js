...
router.get('/:id', [], async (req, res) => {
  try {
    ...
    if (!article) {
      return res.status(404).json({ msg: 'Article not found ! ┐(´д｀)┌' });
    }
...
router.get('/user/:userId', auth, async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ msg: 'User author not found ! ┐(´д｀)┌' });
    }
...
router.post(
  ...
  [
    auth,
    [
      check('title', 'Hey, your article needs a title ! ಠ_ಠ').not().isEmpty(),
      check('originalContent', 'Ehrm, looks like you submitted ... emptiness ! (◔ڼ◔)')
        .not()
        .isEmpty(),
      check('topicId', 'Which topic was this again ? 눈_눈').not().isEmpty(),
      check('type', 'You need to specify the type of your submission, you know ? ¬_¬')
        .not()
        .isEmpty(),
    ],
  ],
  async (req, res) => {
    ...
    try {
      const { type, title, originalContent, richLinks, topicId } = req.body;
...
router.put('/:id', auth, async (req, res) => {
  const { world, type, title, slug, status, editedContent, thumbnail, category } = req.body;
  ...
  try {
    ...
    if (article) {
      ...
      if (authenticatedUser.roles !== 'reviewer' && authenticatedUser.roles !== 'admin') {
        return res.status(401).json({ msg: 'User not authorized >:[ ' });
      }
